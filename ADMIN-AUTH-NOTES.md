# Admin auth — required before deploying phase 4
...
The personalisation ids are 128-bit random, so they aren't guessable — but an
...
$pw
Add-Content .env "`nADMIN_USER=alan`nADMIN_PASSWORD=$pw"
```
...
secret. Save the password in your password manager — it's the only copy.

## Behaviour

- Wrong or missing credentials → 401 with a browser login prompt.
- `ADMIN_USER` or `ADMIN_PASSWORD` missing from the environment → **503, all
...
# with credentials → 400/404 from the function itself (not 401)
curl.exe -i -u "alan:<password>" "https://<your-site>/admin/personalisation/print?pid=test"
```

The first must be 401. If it isn't, the edge function isn't deployed — check
